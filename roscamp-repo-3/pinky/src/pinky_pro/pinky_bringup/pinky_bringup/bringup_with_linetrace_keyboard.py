...
VAL_MIN = 300       
VAL_MAX = 3900      
STOP_LINE_VAL = 3500  # 세 센서 모두 이 값 이상이면 가로 정지선 감지
BASE_RPM = 27.0      # 라인 트레이싱 시 기본 전진 속도
KP = 0.005           # 비례 이득 (에러 값의 범위가 크므로 미세 조정)
...
class KeyboardController:
    ...
    def start_robot(self):
        ...
        try:
            ...
            print("[성공] 로봇이 실행되었습니다.")
        except Exception as e:
            print(f"[오류] 로봇 시작 실패: {e}")
            self.running = False
    ...
    def stop_robot(self):
        ...
        try:
            ...
            print("[중지] 로봇이 중지되었습니다.")
        except Exception as e:
            print(f"[오류] 로봇 중지 중 오류: {e}")
...
def main(args=None):
    ...
    try:
        # 키보드 입력을 별도 스레드에서 처리
        keyboard_thread = threading.Thread(target=controller.keyboard_listener, daemon=False)
        keyboard_thread.start()
        keyboard_thread.join()
    except KeyboardInterrupt:
        print("\n[종료] KeyboardInterrupt 감지")
    except Exception as e:
