...
async def receive_ai_coordinates(request: Request):
    """Receive detection coordinates from AI Server."""
    try:
        data = await request.json()
        # In a real app, you might save this to a DB or update a global state
        # For now, we'll log it for verification
        import logging
        logging.info(f"Received coordinates from AI: {data}")
        return {"status": "success", "received": data.get("seq")}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
