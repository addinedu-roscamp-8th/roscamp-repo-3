...
class Pinky(Node):
    def __init__(self):
        ...
        self.virtual_battery = 100.0 # 초기 배터리 충전량 설정
        ...
        self.current_moving_state = "IDLE" # 현재 상태 추적용
        ...
        self.get_logger().info('Pinky Bringup has been started successfully.')
    ...
    def virtual_battery_timer_callback(self):
        ...
        # 4. 배터리 20% 미만 시 경고 로그 (추후 Nav2와 연동 가능)
        if self.virtual_battery <= 20.0:
            self.get_logger().error(f"Low Virtual Battery! Current: {self.virtual_battery}%")
    ...
    def twist_callback(self, msg: Twist):
        ...
        is_moving = abs(rpm_l) > 0.01 or abs(rpm_r) > 0.01
        new_state = "MOVING" if is_moving else "IDLE"

        # 상태가 변했을 때만 DB에 서비스 요청 (중요: 매번 보내지 않음)
        if new_state != self.current_moving_state:
            self.send_db_update(new_state)
            self.current_moving_state = new_state
    ...
    def send_db_update(self, state):
        if not self.db_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn('DB 서비스를 찾을 수 없어 업데이트를 스킵합니다.')
            return

        req = UpdateStatus.Request()
        req.robot_id = 'Pinky_d9ec'
        req.status = state
        
        # 비동기 호출로 제어 루프 방해 방지
        future = self.db_client.call_async(req)
        # 응답 확인이 꼭 필요하면 콜백을 추가할 수 있습니다.
