...
## ⚠️ 자문자답 (FAQ)
- **Q: 대시보드에 로봇 정보가 안 나타나요.**
  - A: [Step 3] 초기 위치 설정을 수행했는지 확인하세요. 맵 상에 점을 찍어주기 전엔 위치 데이터가 발행되지 않습니다.
- **Q: 관제 PC와 연결이 안 돼요.**
  - A: 관제 PC와 로봇이 **같은 와이파이(SSID)**를 사용하고 있는지 확인하세요.
- **Q: 대시보드 화면이 깨져요.**
  - A: 터미널 창을 충분히 넓게 키운 뒤 다시 실행해 보세요.
