...
class FactoryIntegratedControl(Node):
    def __init__(self):
        ...
        for i in range(1, 4):
            ...
            # 핑키 경로 수신 (교통 관리용)
            self.create_subscription(String, f'/pinky{i}/path', lambda msg, idx=i-1: self.p_path_cb(msg, idx), 10)
    ...
    # 교통 관리 (Nav2 구간용)
    def p_path_cb(self, msg, idx):
        # 여기서는 단순화를 위해 항상 GO를 보내거나, 기존 TrafficManager 로직을 연결할 수 있습니다.
        res = String(); res.data = "GO"
        getattr(self, f'pub_p{idx+1}_res').publish(res)
